...
public:
	...
	DynamicObject(glm::vec3 _startPos, glm::vec3 _scale, float _mass, float _bounciness, glm::vec3 _size, std::string _texName, std::string _type, std::string _tag);
	~DynamicObject();


	virtual void update(float _deltaTs, std::vector<GameObject *> _objects, int _i);
	...
	void collide(float _deltaTs, std::vector<GameObject *> _objects, int _i);
